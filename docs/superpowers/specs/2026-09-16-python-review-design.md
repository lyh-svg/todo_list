...
3. **规模**：第 1 周合并后 **40 个知识点 / ≥200 道题**（五类题是下限，按覆盖优先可加变体）；全 8 周约 **250 个知识点（230～290）/ 约 1300～1600 道题**。估算方法：真实任务文本里的并列机制合计 502 个，按第 1 周实测的合并率（81 个候选 → 40 点）折算，每周再补隐含前置，第 8 周综合周只出综合题。粒度定在 10～30 分钟时是这个量级；若按最早的 5～15 分钟粒度会接近翻倍（约 450～500 点）。
...
| --- | --- | --- | --- | --- |
| 第 7 批 | 第 1 周 基线诊断与基础语义（**基础层**） | 21 | **40** | **≥200** |
| 第 8 批 | 第 2 周 对象模型与协议 | 21 | ~36 | ~180 |
| 第 9 批 | 第 3 周 函数、闭包与装饰器 | 21 | ~33 | ~165 |
| 第 10 批 | 第 4 周 迭代器、生成器与惰性流水线 | 21 | ~34 | ~170 |
| 第 11 批 | 第 5 周 类型、测试与工程化 | 21 | ~36 | ~180 |
| 第 12 批 | 第 6 周 线程、进程与并发边界（**进阶层**） | 21 | ~33 | ~165 |
| 第 13 批 | 第 7 周 asyncio 与结构化并发（**进阶层**） | 21 | ~31 | ~155 |
| 第 14 批 | 第 8 周 整合、质量与最终验收（综合题为主） | 21 | ~15 | ~75 |
| **合计** | 8 周 | **168** | **约 250（230～290）** | **约 1300（下限）** |

三层 ↔ 你的 8 周：**第一层（基础）= 第 1 周（40 点）**；**第二层（实用）= 第 2～5 周（约 139 点）**；**第三层（进阶）= 第 6～7 周（约 64 点，按目标选做）**；第 8 周（约 15 点）只出综合题，不新造知识点。

时间量级：250 点 × 约 20 分钟 ≈ **85 小时**内容；复习按每天 5～15 题，把一周的内容过完一轮大约 2～3 周（之后靠间隔重复维持）。
